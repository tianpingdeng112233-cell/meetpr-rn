...
| TrainingHistory | 🔨 | W1-g v2 复核：六块顺序、e1RM 四态/范围循环、去重训练日/ISO周/容量与三次解锁、反馈归档、历史锁/骨架/失败重试、v3 tokens+i18n；按 202e95db 无调用点删除 growth-curve 路由，Dashboard 转 Growth。静态/单测验证详见 CODEX-JOURNAL；ADB sandbox listener 权限阻断，模拟器截图与 W3 像素对齐待验 |
