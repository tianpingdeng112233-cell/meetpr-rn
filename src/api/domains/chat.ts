...
export const chatRepository = {
  ...
  sendSetRef: (id: string, input: { body: string; clientID: string; setRef: ChatSetRef; videoId?: string }) => authenticatedRequest(`/conversations/${UuidSchema.parse(id)}/messages`, { method: 'POST', body: { kind: 'text', body: z.string().min(1).max(4000).parse(input.body), client_id: z.string().min(1).parse(input.clientID), set_ref: ChatSetRefSchema.parse(input.setRef), ...(input.videoId == null ? {} : { video_id: UuidSchema.parse(input.videoId) }) }, schema: z.object({ message: ChatMessageSchema }) }),
