...
export function StudentOnboardingProfileView() {
  ...
  const actionRow = <View style={[rowStyle, { gap: spacing.point9 }]}><Action testID="coach.applicationProfile.accept" label={t('coach.applicationProfile.accept')} filled onPress={() => actions.openAccept(item)} disabled={actions.busy} style={{ flex: 1 }} /><Action testID="coach.applicationProfile.ignore" label={t('coach.applicationProfile.ignore')} onPress={() => actions.reject(item)} disabled={actions.busy} /></View>;
  return <Screen><View style={[rowStyle, { gap: spacing.space3, paddingHorizontal: spacing.point18, paddingTop: spacing.space1, paddingBottom: spacing.point14, borderBottomWidth: 1, borderColor: colors.borderDefault }]}><Action testID="coach.applicationProfile.back" label={t('coach.applicationProfile.back')} icon="chevron-back" onPress={() => router.back()} style={{ width: 44, paddingHorizontal: 0, backgroundColor: colors.surfaceCard, borderWidth: 0 }} /><View style={{ flex: 1, gap: spacing.point2 }}><Copy size={16} weight="bold" lines={1}>{t('coach.applicationProfile.title', [item.displayName])}</Copy><Copy size={11} tone="textTertiary">{waitingText(item.submittedAt, now)}</Copy></View></View>
