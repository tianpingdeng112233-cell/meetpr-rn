...
export function GrowthE1RMCard({ curve, isZeroTraining, onToday, onSelect, onRangeChange, selectedPointId }: { selectedPointId?: string | null; curve: GrowthCurve; isZeroTraining: boolean; onToday: () => void; onSelect?: (sample: E1RMSample) => void; onRangeChange?: () => void }) {
  ...
  return <Card style={{ paddingHorizontal: spacing.space4, paddingVertical: spacing.space4, elevation: 0, shadowOpacity: 0 }}>
    ...
    {snapshot.state === 'chart' ? <View style={{ paddingTop: spacing.space2 }}><GrowthE1RMChart selectedPointId={selectedPointId} onSelect={onSelect} samples={snapshot.samples} rawEligiblePoints={snapshot.rawEligiblePoints} /><Text style={{ ...font.body(11), color: colors.textMuted, marginTop: 8, textAlign: 'center' }}>{t('student.e1rmSourceHint')}</Text></View> : snapshot.state === 'formingWindowSparse' ? <View style={{ height: 126, justifyContent: 'center' }}>
