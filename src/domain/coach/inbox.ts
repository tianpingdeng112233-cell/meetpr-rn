...
export type InboxConversation = { id: string; otherPartyID: string; otherPartyName: string; unreadCount: number; lastMessageAt: string | null; lastMessagePreview: string | null };
...
export function inboxRows({ conversations, videoGroups, now }: { conversations: readonly InboxConversation[]; videoGroups: readonly VideoGroup[]; now: Date }) {
  ...
  return [...new Set([...chats.keys(), ...videos.keys()])].map(studentID => {
    ...
    return {
      ...
      lastPreview: videoGroup ? t('coach.inbox.pendingVideoPreview %lld %@', [videoGroup.count, relativeText(videoGroup.latestUploadedAt, now)]) : conversation?.lastMessagePreview ?? t('coach.inbox.noMessages'),
    };
